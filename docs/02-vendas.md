...
- **Vendas líquidas seguem a mesma regra de devolução do Painel Mensal** (31/07/2026): a devolução vem da **tabela `devolucoes`** (`SUM(d.VL_TOTAL_LIQ)`, custo `SUM(d.QUANTIDADE × produtos.CUSTO)`) e é abatida no **mês da venda original** (`vendas.DATA`), do **vendedor da venda original** e da **faixa do produto devolvido**, ignorando vendas canceladas (`STATUS='3'`) — exatamente como `atualizar_mapa.py`. O bruto vem de `SUM(i.VL_UNIT × i.QUANTIDADE)` / `SUM(i.CUSTO × i.QUANTIDADE)`; `MC = vendas − custo`.
  - *Antes* usava a netagem na própria linha de `itens` (`QUANTIDADE − QTD_DEVOLVIDA − QTD_ESTORNADA`), como o CMV e o pico ainda fazem. As duas fontes divergem de 5% a 15% no valor devolvido por mês, então a página não batia com o Painel Mensal.
  - **Inconsistência conhecida remanescente:** esta página soma o item a **preço cheio** (`VL_UNIT × QUANTIDADE`), sem abater `itens.DESCONTO`, enquanto o Painel Mensal usa `vendas.VL_TOTAL` (já líquido de desconto). Isso deixa as vendas ~7% acima do Painel Mensal (jul/2026: R$ 249.466 vs R$ 232.913) e **infla a comissão apurada na mesma proporção**. Abater `i.DESCONTO` reconcilia as duas visões a 0,02%. Mantido a preço cheio por ora — é decisão de negócio se a comissão incide sobre a venda bruta ou líquida de desconto.
